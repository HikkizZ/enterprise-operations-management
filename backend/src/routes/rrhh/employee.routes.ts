...
router.get('/:id', authenticateJWT, verifySelfOrRole([userRoles.RECURSOS_HUMANOS, userRoles.GERENCIA]), getEmployeeByIdController);
router.post('/', authenticateJWT, verifyRole([userRoles.RECURSOS_HUMANOS]), createEmployeeController);
router.patch('/:id', authenticateJWT, verifySelfOrRole([userRoles.RECURSOS_HUMANOS]), updateEmployeeController);
