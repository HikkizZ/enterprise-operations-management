...
router.post('/:id', authenticateJWT, verifyRole([userRoles.RECURSOS_HUMANOS]), uploadPdf('leaves'), createLeaveController);
router.get('/employee/:id', authenticateJWT, verifyRole([userRoles.RECURSOS_HUMANOS, userRoles.GERENCIA]), getLeavesByEmployeeIdController);
...
router.patch('/:leaveId/cancel', authenticateJWT, verifyRole([userRoles.RECURSOS_HUMANOS]), cancelLeaveController);
